...
//get token
app.get("/me",auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async(req,res)=>{
...
//get user by id
app.get('/:id',auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]), async (req, res) => {
  const id = req.params.id;
...
//update user
app.put('/:id', auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async (req, res) => {
  const id = req.params.id;

  let password = bcrypt.hashSync(req.body.user.tcNo.substr(5), Number.parseInt(authConfig.rounds));
  req.body.user.password = password
  db.User.update(req.body.user, {where: {id: id },paranoid: false}).then(result =>{
    return res.json({
      type: true,
      data: "User Updated"
    });
...
app.post("/new",async(req,res)=>{
  ...
  let password = bcrypt.hashSync(data.tcNo.substr(5), Number.parseInt(authConfig.rounds));

  // Create a user,
  db.User.findOne({where: {tcNo : data.tcNo  },paranoid: false}).then(user =>{
...
// getting tip admins
app.get('/tip/admins',auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async (req, res) => {
...
app.get('/monthly-info/:deviceIdNo',auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async (req, res) => {
...
app.get('/invidual-info/:deviceIdNo',auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async (req, res) => {
...
app.get('/input-output/:deviceIdNo',auth([UserRolls.Admin,UserRolls.Trainer,UserRolls.CompanyAdmin]),async (req, res) => {
